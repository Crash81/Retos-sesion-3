...
CREATE VIEW puesto_empleado_012 AS SELECT empleado.nombre,apellido_paterno,apellido_materno,puesto.nombre FROM empleado JOIN puesto ON empleado.id_puesto=puesto.id_puesto;
...
SELECT * FROM venta_articulo_empleado;
...
SELECT * FROM puesto_venta WHERE venta=(SELECT max(venta) FROM puesto_venta);